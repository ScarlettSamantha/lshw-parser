...
final class ParserTest extends TestCase
{
    ...
    protected string $validFixtureFile = "lshw-output.xml";
    protected string $invalidFixtureFile = "invalid.xml";
    ...
    protected function getValidFixturePath(): string
    {
        return $this->getFixturePath($this->validFixtureFile);
    }

    protected function getInvalidFixturePath(): string
    {
        return $this->getFixturePath($this->invalidFixtureFile);
    }

    protected function setUp(): void
    {
        $this->xmlContent = file_get_contents($this->getValidFixturePath());
    }
    ...
    public function testConstructorWithXmlFile(): void
    {
        ...
        $parser = new Parser($this->getValidFixturePath());
        $this->assertInstanceOf(Parser::class, $parser);
    }
    ...
    public function testConstructorWithNonExistentFile(): void
    {
        ...
        new Parser($this->getInvalidFixturePath());
    }

    public function testExceptionWhenUnableToReadFile(): void
    {
        $this->expectException(ParserException::class);

        // Create a mock for the Parser class
        $parser = $this->getMockBuilder(Parser::class)
            ->setConstructorArgs([$this->getInvalidFixturePath()])
            ->onlyMethods(['readFile'])
            ->getMock();

        // Configure the mock to return false for readFileContent
        $parser->method('readFile')->willReturn(false);

        // Expect the ParserException to be thrown

        $this->expectException(ParserException::class);
        $this->expectExceptionMessage("Unable to read file: {$this->getInvalidFixturePath()}");

        // Trigger the exception
        $parser->__construct($this->getInvalidFixturePath());
    }
...
XML;
    ...
    public function testGetSkipHubsInitiallyFalse(): void
    {
        // Create a new instance of Parser
        $parser = new Parser($this->getValidFixturePath());
        // Assert that the initial value of skipHubs is false
        $this->assertFalse($parser->getSkipHubs());
    }

    public function testSetSkipHubsTrue(): void
    {
        // Create a new instance of Parser
        $parser = new Parser($this->getValidFixturePath());

        // Set skipHubs to true
        $parser->setSkipHubs(true);

        // Assert that getSkipHubs() returns true
        $this->assertTrue($parser->getSkipHubs());
    }

    public function testSetSkipHubsFalse(): void
    {
        // Create a new instance of Parser
        $parser = new Parser($this->getValidFixturePath());

        // Set skipHubs to true initially
        $parser->setSkipHubs(true);

        // Change skipHubs to false
        $parser->setSkipHubs(false);

        // Assert that getSkipHubs() returns false
        $this->assertFalse($parser->getSkipHubs());
    }
}
