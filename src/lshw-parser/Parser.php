...
class Parser
{
    ...
    public function __construct(string $xmlContent, bool $skipHubs = false)
    {
        ...
        if (is_file($xmlContent) && is_readable($xmlContent)) {
            $fileXmlContent = $this->readFile($xmlContent);
     ...
     * 
     * @note This is a proxy function for mocking (cant mock returns of a global php function).
     * @param string $path
     * @return string
     */
    public function readFile(string $path): string
    {
        $fileXmlContent = file_get_contents($path);
        if ($fileXmlContent === false) {
            throw new ParserException("Unable to read file: $path");
        }
        return $fileXmlContent;
    }
